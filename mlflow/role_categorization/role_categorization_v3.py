...
def find_best_k(X_scaled, max_k):
...
class TeamRoleClusteringModel(PythonModel):
    ...
    def __init__(self, kmeans, scaler, role_mapping, feature_names):
        self.kmeans = kmeans
        self.scaler = scaler
...
def main():
    ...
    try:
        ...
        with mlflow.start_run(run_name="K-means Role Categorization Version 3") as run:
            df = preprocess_data(df)

            features = ["avg_workload", "team_work", "work_speed", "overall_quality_score"]
            X_scaled, scaler = standardize_features(df, features)

            scores = find_best_k(X_scaled, min(20, len(df) - 1))
